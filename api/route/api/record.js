...
router.delete('/:id',verifyTokenAndAdmin,async(req,res)=>{
    try{
        //Find Record
        const record_id = Record.findById(req.params.id)
        if(!record_id) return res.status(400).json({ok:0,msg:`No this record.`})

        const deletedSensorrecord = await Sensorrecord.deleteMany({record:req.params.id})
        if(deletedSensorrecord.deletedCount < 1) return res.status(400).json({ok:0,msg:`No this record.`})
        ...
        if(!deletedRecord) return res.status(400).json({ok:0,msg:`No this record.`})
