...
router.delete('/:id',verifyTokenAndAdmin,async(req,res)=>{
    try{
        //Find fishtype
        const fishtype_id = await Fishtype.findById(req.params.id)
        if(!fishtype_id) return res.status(400).json({ok:0,msg:`No this Fishtype.`})
        
        const periods = await Period.find({fishtype : req.params.id}).sort({_id:-1})
        if(periods.length > 0){
            ...
            const deletedPeriod = await Period.deleteMany({fishtype : req.params.id})
