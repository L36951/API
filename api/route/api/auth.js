...
router.post('/login', async (req,res)=>{
    try{
        ...
        res.setHeader("Set-Cookie",`refreshToken=${refreshToken}; Path=/; HttpOnly; max-age=${process.env.COOKIE_EXPIRES_IN};`)

        res.status(201).send({ok: 1,token:accessToken});
...
router.post('/check',verifyToken,async (req,res)=>{
    ...
    return res.status(201).json({ok:1,msg:`${user.username}'s token is still valid`,isAdmin:user.isAdmin})
})
