...
const range = require('../email/range')
const allrange = require('../email/allrange')
...
router.get('/range',async(req,res)=>{
    if(req.query.sensortype){
        const sensortype = await Sensortype.findOne({sensortype : req.query.sensortype})
        if(!sensortype) return res.status(400).json({ok:0,msg:"No this sensortype"})
        const sensortype_range = await range(req.query.sensortype)
        sensortype_range['sensortype'] = req.query.sensortype    
        console.log(`${moment().format('MMMM Do YYYY, h:mm:ss a')} : query ${req.query.sensortype} suitable range`)
        return res.status(200).json({ok:1,data:sensortype_range})
    }
    const arange = await allrange()
    console.log(`${moment().format('MMMM Do YYYY, h:mm:ss a')} : query all suitable range`)
    return res.status(200).json({ok:1,data:arange})
})
...
router.get('/queryend/aver',verifyTokenAndAdmin,async(req,res)=>{
    ...
    for(let i=0;i<periods.length;i++){ 
        ...
        tempArray.push({'end_qty': periods[i].end_quantity})
/*
...
router.get('/daydata',async(req,res)=>{
